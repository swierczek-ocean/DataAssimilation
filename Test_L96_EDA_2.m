...
L_EDA = ACC_Localize(n,r2);             % EDA localization matrix for covariance
%%
...
for kk=q_split+1:q  
    ...
    [X_star_t_EDA,spread,CovEDA,Time_Series] = DA_EDA(X_star_t_EDA,L96fun,...
        gradient_fun,CovEDA,H,X_star_t_EDA,dt,jump,Obs,ObsVar,n,L_EDA,alpha2,Ne_EDA);
...
h1 = plot(ErrorVecEDA,'Color',Color(:,color1),'LineWidth',2.2);
hold on
h2 = plot(spreadVecEDA,'Color',Color(:,color2),'LineWidth',2.2);
title('RMSE & spread')
xlabel('time')
ylabel('RMSE')
legend([h1(1),h2(1)],'4DVar','spread')
print('Test_L96_EDA_2','-djpeg')
hold off
%%
