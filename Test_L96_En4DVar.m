...
TimeSeriesEn4DVar = zeros(n,exp_iter);         % array for storing full DA
total_steps = 0;
...
Obs = H*Time_Series_True(:,num_steps);
    [Ensemble,X_star_t,spread,Time_Series,Cov] = DA_En4DVar(X_star_t,Ensemble,...
        L96fun,gradient_fun,H,BCov,mu,dt,jump,Obs,ObsVar,n,L,alpha);
...
for kk=2:q    
    ...
    [Ensemble,X_star_t,spread,Time_Series,Cov] = DA_En4DVar(X_star_t,Ensemble,...
        L96fun,gradient_fun,H,Cov,mu,dt,jump,Obs,ObsVar,n,L,alpha);
...
TimeSeriesEn4DVar(:,end) = X_star_t;
...
error_parameter = mean(ErrorVecEn4DVar(10*jump:end));
fprintf('Average RMSE: %g\n',error_parameter)
fprintf('Average spread: %g\n',mean(spreadVecEn4DVar(10:end)))

%% error plot
set(gcf, 'Position', [25, 25, 1600, 900])
h1 = plot(spreadVecEn4DVar,'Color',Color(:,color1),'LineWidth',2.2);
hold on
h2 = plot(ErrorVecEn4DVar,'Color',Color(:,color2),'LineWidth',2.2);
title('RMSE & spread for 4DVar')
% axis([0 q 0 3*error_parameter])
xlabel('time')
legend([h1(1),h2(1)],'spread','RMSE')
print('Test_L96_En4DVar','-djpeg')
hold off
%%
